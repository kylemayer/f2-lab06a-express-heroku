...
it('ducks responds with all of the ducks', async() => {
    const response = await request.get('/ducks');
    expect(response.body).toEqual(ducks);
});

it('ducks/:id responds with a single duck', async() => {
    const response = await request.get('/ducks/7');
    expect(response.body).toEqual(wood);
});